...
data_dir = f"{base_dir}/wdbc"
batch_dir = f"{data_dir}/wisconsin"

output_bronze = f"{database_dir}/breastcancer_facts/bronze"
output_silver = f"{database_dir}/breastcancer_facts/silver"
...
# use spark dataframe to create a new table in the databricks metadata database
get_ipython().run_line_magic('scala', '')
...
# use pyspark to read from a csv file
wisconsin_csv = f"{batch_dir}/breast-cancer-wisconsin.csv"

df_wisconsin = spark.read.format('csv').options(header='true', inferSchema='true').load(address_csv)
display(df_wisconsin)
